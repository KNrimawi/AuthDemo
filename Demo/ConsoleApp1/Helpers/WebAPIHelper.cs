...
namespace ConsoleApp1.Helpers
{
    public class WebAPIHelper
    {
        ...
        public T Get<T>(string uriBaseAddress, string uriPath, Dictionary<string, string> parameters, Dictionary<string, List<string>> parametersLists, Dictionary<string, string> headers, string jwt, string mediaType = "application/json")
        {
            try
            {
                ...
                using (var handler = new HttpClientHandler() { CookieContainer = cookieContainer})
                {
                    ...
                    using (var client = new HttpClient(handler))
                    {
                        ...
                        cookieContainer.Add(uriBuilder.Uri, new Cookie("JWT", jwt) { HttpOnly=true});
                        // sending the request
